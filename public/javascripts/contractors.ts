...
;(() => {
  ...
  const getTradeCategoryFromCache = (tradeCategoryID: number) => {
    const tradeCategory = updateOptionsCache.tradeCategories.find(
      (potentialTradeCategory) => {
  ...
  const buildVendorInformationSystemURL = (contractorID: number) => {
    return (
      exports.vendorInformationSystemVendorURL +
      '?vendorID=' +
  ...
  const openDocuShareLink = (clickEvent: Event) => {
    ...
    const docuShareCollectionID = (
      document.querySelector(
        '#contractor--docuShareCollectionID'
      ) as HTMLInputElement
    ).value

    if (docuShareCollectionID === '') {
      cityssm.alertModal(
        'DocuShare Collection ID Not Available',
  ...
  type UpdateFormActions =
    | 'doUpdateContractor'
    | 'doUpdateHealthSafety'
    | 'doUpdateLegal'
    | 'doUpdateWSIB'
    | 'doUpdateInsurance'
  ...
  const submitUpdateForm = (formEvent: Event) => {
    ...
    const updateFormAction = formElement.getAttribute(
      'data-action'
    ) as UpdateFormActions

    cityssm.postJSON(
      urlPrefix + '/contractors/' + updateFormAction,
      formElement,
  ...
  const submitAddTradeCategoryForm = (formEvent: Event) => {
    ...
    const tradeCategoryID = (
      document.querySelector(
        '#tradeCategories--tradeCategoryID'
      ) as HTMLSelectElement
    ).value

    if (tradeCategoryID === '') {
      cityssm.alertModal(
        'No Trade Category Selected',
      ...
      cityssm.alertModal(
        'Trade Category Already Included',
    ...
    cityssm.postJSON(
      urlPrefix + '/contractors/doAddTradeCategory',
      formElement,
      (responseJSON: { success: boolean }) => {
        if (responseJSON.success) {
          const contractorID = Number.parseInt(
            (
          ...
          const tradeCategory = getTradeCategoryFromCache(
            Number.parseInt(tradeCategoryID, 10)
          )
          const tradeCategoryElement = buildContractorTradeCategoryElement(
            contractorID,
            tradeCategory
          )

          const tradeCategoriesContainerElement = document.querySelector(
  ...
  function createDocuShareCollection(clickEvent: MouseEvent): void {
    const createButtonElement = clickEvent.currentTarget as HTMLButtonElement

    const docuShareCollectionIDElement = document.querySelector(
      '#contractor--docuShareCollectionID'
    ) as HTMLInputElement
    ...
    const doCreate = () => {
      const contractorID =
        createButtonElement.getAttribute('data-contractor-id')

      cityssm.postJSON(
        urlPrefix + '/contractors/doCreateDocuShareCollection',
        {
          contractorID
        },
        (responseJSON: {
          ...
          if (responseJSON.success) {
            docuShareCollectionIDElement.value =
              responseJSON.docuShareCollectionID
          } else {
            cityssm.alertModal(
              'DocuShare Collection Not Created',
    ...
    if (currentDocuShareCollectionID === '') {
      cityssm.confirmModal(
        'Create a New Collection in DocuShare',
  ...
  const removeTradeCategory = (clickEvent: MouseEvent) => {
    ...
    const removeFunction = () => {
      const contractorID =
        deleteButtonElement.getAttribute('data-contractor-id')

      const tradeCategoryID = deleteButtonElement.getAttribute(
        'data-trade-category-id'
      )

      cityssm.postJSON(
        urlPrefix + '/contractors/doRemoveTradeCategory',
        {
          contractorID,
          tradeCategoryID
        },
  ...
  const buildContractorTradeCategoryElement = (
    contractorID: number,
    tradeCategory: recordTypes.TradeCategory
  ) => {
    const tradeCategoryElement = document.createElement('div')
    tradeCategoryElement.className = 'panel-block'

    tradeCategoryElement.innerHTML =
    ...
    if (canUpdate) {
      ...
      deleteButtonElement.className =
        'button is-small is-danger is-inverted is-edit-control-flex'
      deleteButtonElement.type = 'button'

      deleteButtonElement.dataset.contractorId = contractorID.toString()
      deleteButtonElement.dataset.tradeCategoryId =
        tradeCategory.tradeCategoryID.toString()
  ...
  const loadTradeCategoryOptions = () => {
    ...
    if (updateOptionsCache.tradeCategories.length > 0) {
      ...
      cityssm.postJSON(
        urlPrefix + '/contractors/doGetAllTradeCategories',
        {},
  ...
  const loadHealthSafetyOptions = () => {
    const renderFunction = () => {
      ...
      for (const status of updateOptionsCache.healthSafetyStatuses) {
        optgroupElement.insertAdjacentHTML(
          'beforeend',
      ...
      document
        .querySelector('#contractor--healthSafety_status')
        .append(optgroupElement)
    }
    ...
    if (updateOptionsCache.healthSafetyStatuses.length > 0) {
      ...
      cityssm.postJSON(
        urlPrefix + '/contractors/doGetHealthSafetyOptions',
        {},
        (responseJSON: { healthSafetyStatuses: string[] }) => {
          updateOptionsCache.healthSafetyStatuses =
            responseJSON.healthSafetyStatuses
          renderFunction()
        }
      )
    }
  ...
  const loadInsuranceOptions = () => {
    const renderFunction = () => {
      const datalistElement = document.querySelector(
        '#contractor--insurance_company-datalist'
      )

      for (const companyName of updateOptionsCache.insuranceCompanyNames) {
        datalistElement.insertAdjacentHTML(
          'beforeend',
    ...
    if (updateOptionsCache.insuranceCompanyNames.length > 0) {
      ...
      cityssm.postJSON(
        urlPrefix + '/contractors/doGetInsuranceOptions',
        {},
        (responseJSON: { insuranceCompanyNames: string[] }) => {
          updateOptionsCache.insuranceCompanyNames =
            responseJSON.insuranceCompanyNames
          renderFunction()
        }
      )
    }
  ...
  const unlockContractorUpdateForm = (clickEvent: MouseEvent) => {
    ...
    const createButtonElement = document.querySelector(
      '#contractor--docuShareCollectionID-create'
    ) as HTMLButtonElement
    createButtonElement.addEventListener('click', createDocuShareCollection)
    createButtonElement.dataset.contractorId = (
      formElement.querySelector('.contractor--contractorID') as HTMLInputElement
    ).value
    ;(
      document.querySelector(
        '#contractor--docuShareCollectionID'
  ...
  const unlockTradeCategoriesUpdateForm = (clickEvent: MouseEvent) => {
    const unlockContainerElement = (
      clickEvent.currentTarget as HTMLElement
    ).closest('.is-unlock-tradecategories-container')
    const formElement = document.querySelector('#form--tradeCategories')

    document
      .querySelector('#contractor--tradeCategories')
      .classList.add('is-edit-mode')
  ...
  const openContractorModal = (clickEvent: MouseEvent) => {
    ...
    const contractorIndex: number = Number.parseInt(
      (clickEvent.currentTarget as HTMLAnchorElement).getAttribute(
        'data-index'
      ),
      10
    )
    ...
    const loadTradeCategories = () => {
      ...
      cityssm.postJSON(
        urlPrefix + '/contractors/doGetTradeCategoriesByContractorID',
        {
          contractorID: contractor.contractorID
        },
        (responseJSON: { tradeCategories: recordTypes.TradeCategory[] }) => {
          const tradeCategoriesElement = document.querySelector(
            '#contractor--tradeCategories'
          )
    ...
    cityssm.openHtmlModal('contractor-view', {
      onshow: (modalElement) => {
        ...
        document.querySelector('#contractor--contractor_name').textContent =
        ...
        document.querySelector('#contractor--email_address').innerHTML =
          contractor.email_address && contractor.email_address !== ''
            ? '<a href="mailto:' +
              cityssm.escapeHTML(contractor.email_address) +
              '">' +
        ...
        if (contractor.docuShareCollectionID) {
          ...
          ).value = contractor.docuShareCollectionID.toString()
        }

        document
          .querySelector('#contractor--docuShareCollectionID-link')
          .addEventListener('click', openDocuShareLink)

        if (canUpdate) {
          const contractorIDElement = document.querySelector(
            '#contractor--contractorID'
          )
          contractorIDElement.innerHTML =
            '<a href="' +
            buildVendorInformationSystemURL(contractor.contractorID) +
            '" target="_blank">' +
        ...
        ;(
          document.querySelector(
            '#contractor--healthSafety_status'
          ) as HTMLSelectElement
        ).innerHTML =
          contractor.healthSafety_status === null ||
          contractor.healthSafety_status === ''
            ? '<option value="">(Not Set)</option>'
            : '<option>' +
              cityssm.escapeHTML(contractor.healthSafety_status) +
              '</option>'
        ...
        ).innerHTML = legalOptionHTML
        ;(
          document.querySelector(
            '#contractor--legal_note'
          ) as HTMLTextAreaElement
        ).value = contractor.legal_note
        ...
        ).value = contractor.wsib_accountNumber
        ;(
          document.querySelector(
            '#contractor--wsib_firmNumber'
          ) as HTMLInputElement
        ).value = contractor.wsib_firmNumber
        ...
        ).value = contractor.insurance_company || ''
        ;(
          document.querySelector(
            '#contractor--insurance_policyNumber'
          ) as HTMLInputElement
        ).value = contractor.insurance_policyNumber || ''
        ;(
          document.querySelector(
            '#contractor--insurance_amount'
          ) as HTMLInputElement
        ).value = contractor.insurance_amount.toString()
        ...
        if (contractor.insurance_expiryDate) {
          ...
          ;(
            document.querySelector(
              '#contractor--insurance_expiryDate'
            ) as HTMLInputElement
          ).value = expiryDateString
        }
        ...
        if (canUpdate) {
          const contractorIDInputElements = modalElement.querySelectorAll(
            '.contractor--contractorID'
      ...
      onshown: (modalElement) => {
        const legalCriteriaAliasElements = modalElement.querySelectorAll(
          '.uses-legal-criteria-alias'
        )
        ...
        if (canUpdate) {
          ...
          const contractorUnlockControlElement = modalElement.querySelector(
            '.is-unlock-contractor-control'
          )

          contractorUnlockControlElement.classList.remove('is-hidden')

          contractorUnlockControlElement
            .querySelector('button')
            .addEventListener('click', unlockContractorUpdateForm)

          // Trade Categories Form

          const tradeCategoriesUnlockContainerElement =
            modalElement.querySelectorAll(
              '.is-unlock-tradecategories-container'
            )[0]

          tradeCategoriesUnlockContainerElement.classList.remove('is-hidden')

          tradeCategoriesUnlockContainerElement
            .querySelector('button')
  ...
  function isContractorHireReady(contractor: recordTypes.Contractor): boolean {
    return (
  ...
  const buildContractorHealthSafetyIconHTML = (
    contractor: recordTypes.Contractor
  ): string => {
    const html =
      '<span class="icon">' +
      (contractor.healthSafety_isSatisfactory
  ...
  function buildContractorAccessibilityIconHTML(
    contractor: recordTypes.Contractor
  ): string {
    const html =
      '<span class="icon">' +
      (contractor.accessibility_isSatisfactory
        ? '<i class="fas fa-2x fa-check-circle has-text-success" aria-hidden="true"></i>'
        : '<i class="fas fa-2x fa-times-circle has-text-danger" aria-hidden="true"></i>') +
      '</span><br />' +
      '<span class="is-size-7 has-text-weight-semibold">Accessibility</span>'

    return html
  }

  const buildContractorWSIBIconHTML = (
    contractor: recordTypes.Contractor
  ): string => {
    let tooltipText = ''
    ...
    if (contractor.wsib_isIndependent) {
      ...
      tooltipText =
        'Expires ' + cityssm.dateToString(new Date(contractor.wsib_expiryDate))
    }
  ...
  const buildContractorInsuranceIconHTML = (
    contractor: recordTypes.Contractor
  ): string => {
    let tooltipText = ''
    ...
    if (contractor.insurance_isSatisfactory) {
      tooltipText =
        'Expires ' +
        cityssm.dateToString(new Date(contractor.insurance_expiryDate))
  ...
  function buildContractorContactDetails(
    contractor: recordTypes.Contractor
  ): string {
    return (
      (contractor.phone_number
        ? `<span class="icon"><i class="fas fa-phone" aria-hidden="true"></i></span>
            <span aria-label="Phone Number">${contractor.phone_number}</span><br />`
        : '') +
      (contractor.email_address
        ? `<span class="icon"><i class="fas fa-envelope" aria-hidden="true"></i></span>
            <span aria-label="Email Address">${contractor.email_address}</span><br />`
        : '')
    )
  }

  const buildContractorResultElement = (
    contractor: recordTypes.Contractor,
    contractorIndex: number
  ): HTMLElement => {
    const panelBlockElement = document.createElement('div')
    panelBlockElement.className = 'panel-block is-block'
    panelBlockElement.style.pageBreakInside = 'avoid'
    ...
    columnsElement.innerHTML = `<div class="column is-full-mobile is-full-tablet is-half-widescreen">
      <a class="is-size-5 has-text-weight-bold" data-index="${contractorIndex.toString()}" role="button" href="#">
        ${cityssm.escapeHTML(contractor.contractor_name)}
      </a><br />
      ${
      ...
      <div class="column pt-4 has-text-centered">${buildContractorHealthSafetyIconHTML(contractor)}</div>
      <div class="column pt-4 has-text-centered">${buildContractorWSIBIconHTML(contractor)}</div>
      <div class="column pt-4 has-text-centered">${buildContractorInsuranceIconHTML(contractor)}</div>
      <div class="column pt-4 has-text-centered">${buildContractorAccessibilityIconHTML(contractor)}</div>`

    columnsElement
      .querySelector('a')
      ?.addEventListener('click', openContractorModal)
  ...
  function getContractors(): void {
    ...
    cityssm.postJSON(
      urlPrefix + '/contractors/doGetContractors',
      filterFormElement,
      (responseJSON: { contractors: recordTypes.Contractor[] }) => {
        ...
        for (const [contractorIndex, contractor] of contractors.entries()) {
          const panelBlockElement = buildContractorResultElement(
            contractor,
            contractorIndex
          )
        ...
        if (hireReadyCount === 0) {
          hireReadyHTML +=
            '<span class="has-text-danger">No Displayed Contractors are Hire Ready</span>'
        } else if (hireReadyCount === contractors.length) {
          hireReadyHTML +=
            '<span class="has-text-success">All Displayed Contractors are Hire Ready</span>'
        } else {
          hireReadyHTML +=
            '<span class="has-text-success">' +
            hireReadyCount.toString() +
            ' out of ' +
            contractors.length.toString() +
            ' Contractors are Hire Ready</span>'
        }
  ...
  document
    .querySelector('#filter--contractorName')
    ?.addEventListener('change', getContractors)
  document
    .querySelector('#filter--tradeCategoryID')
    ?.addEventListener('change', getContractors)
  document
    .querySelector('#filter--hireStatus')
    ?.addEventListener('change', getContractors)
})()
