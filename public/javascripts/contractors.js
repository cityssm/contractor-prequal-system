...
(() => {
    ...
    const buildVendorInformationSystemURL = (contractorID) => {
        return (exports.vendorInformationSystemVendorURL +
            '?vendorID=' +
            contractorID.toString());
    };
    const buildDocuShareURL = (docuShareCollectionID) => {
        return (exports.docuShareRootURL +
    ...
    function createDocuShareCollection(clickEvent) {
    ...
    const loadTradeCategoryOptions = () => {
        const renderFunction = () => {
            ...
            for (const tradeCategory of updateOptionsCache.tradeCategories) {
                selectElement.insertAdjacentHTML('beforeend', '<option value="' +
                    tradeCategory.tradeCategoryID.toString() +
                    '">' +
    ...
    const loadLegalOptions = () => {
        document
            ...
            ?.insertAdjacentHTML('beforeend', '<optgroup label="Options">' +
    ...
    const openContractorModal = (clickEvent) => {
        ...
        cityssm.openHtmlModal('contractor-view', {
            onshow: (modalElement) => {
                ...
                const location = (contractor.contractor_city
                ...
                if (contractor.docuShareCollectionID) {
                    ;
                    document.querySelector('#contractor--docuShareCollectionID').value = contractor.docuShareCollectionID.toString();
                }
                document
                ...
                const legalOptionHTML = contractor.legal_isSatisfactory
                    ? '<option value="1">Approved</option>'
                    : '<option value="0">Declined</option>';
                document.querySelector('#contractor--legal_isSatisfactory').innerHTML = legalOptionHTML;
                document.querySelector('#contractor--legal_note').value = contractor.legal_note;
                document.querySelector('#contractor--wsib_accountNumber').value = contractor.wsib_accountNumber;
                document.querySelector('#contractor--wsib_firmNumber').value = contractor.wsib_firmNumber;
                if (contractor.wsib_effectiveDate) {
                    ...
                    document.querySelector('#contractor--wsib_effectiveDate').value = effectiveDateString;
                }
                if (contractor.wsib_expiryDate) {
                    ...
                    document.querySelector('#contractor--wsib_expiryDate').value = expiryDateString;
                }
                ...
                ;
                document.querySelector('#contractor--insurance_company').value = contractor.insurance_company || '';
                document.querySelector('#contractor--insurance_policyNumber').value = contractor.insurance_policyNumber || '';
                document.querySelector('#contractor--insurance_amount').value = contractor.insurance_amount.toString();
                if (contractor.insurance_expiryDate) {
                    const expiryDate = new Date(contractor.insurance_expiryDate);
                    const expiryDateString = cityssm.dateToString(expiryDate);
                    document.querySelector('#contractor--insurance_expiryDate').value = expiryDateString;
                }
                if (canUpdate) {
    ...
    function isContractorHireReady(contractor) {
    ...
    function buildContractorAccessibilityIconHTML(contractor) {
        const html = '<span class="icon">' +
    ...
    function buildContractorContactDetails(contractor) {
