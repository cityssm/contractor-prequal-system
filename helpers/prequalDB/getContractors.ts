...
const buildWhereClause = (filters: GetContractorFilters) => {
  ...
  if (Object.prototype.hasOwnProperty.call(filters, 'updateYears')) {
    whereClause +=
      ' and datediff(year, updateTime, getDate()) <= ' +
      filters.updateYears.toString()
  }
  ...
  if (Object.prototype.hasOwnProperty.call(filters, 'hireStatus')) {
    switch (filters.hireStatus) {
      ...
      case 'cityApproved':
        // whereClause += " and healthSafety_isSatisfactory = 1" + " and legal_isSatisfactory = 1";
        whereClause += ' and healthSafety_isSatisfactory = 1'

        break

      case 'partiallyApproved':
        // whereClause += " and (healthSafety_isSatisfactory = 1" + " or legal_isSatisfactory = 1)";
        whereClause += ' and healthSafety_isSatisfactory = 1'
        break
    }
  }
  ...
  if (
    Object.prototype.hasOwnProperty.call(filters, 'hasDocuShareCollectionID')
  ) {
  ...
  if (Object.prototype.hasOwnProperty.call(filters, 'tradeCategoryID')) {
    whereClause +=
      " and contractorID in (select cp1b_contractorid from cpqs_p1_business where cp1b_typeid = '" +
      filters.tradeCategoryID.toString() +
      "')"
  }

  if (Object.prototype.hasOwnProperty.call(filters, 'contractorName')) {
    const whereClausePiece = sqlFunctions.buildWhereClauseLike(
      ['contractor_name'],
...
export const getContractors = async (
  ...
  try {
    ...
    const sql = `select contractorID, docuShareCollectionID, isContractor,
      contractor_name, contractor_city, contractor_province, phone_name, phone_title,
      ${
        canUpdate
          ? ' phone_number,'
      ...
      accessibility_status, accessibility_isSatisfactory,
      ${canUpdate ? ' legal_note,' : " '' as legal_note,"}
      legal_isSatisfactory
      from Prequal.Contractor_SearchResults
      ${buildWhereClause(filters)}
      order by contractor_name, contractorID`

    const contractorResult = await pool.request().query(sql)
